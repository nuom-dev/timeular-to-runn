...
export const getProjects = regionalFunctions.https.onCall(
  async (data, context) => {
    return getRunnProjects();
  }
);

export const getRoles = regionalFunctions.https.onCall(
  async (data, context) => {
    return getRunnRoles();
  }
);

export const getUser = regionalFunctions.https.onCall(
  async (data: { email: string }, context) => {
    return getRunnPersonByEmail(data.email);
  }
);

export const setPersonId = regionalFunctions.https.onCall(
  async (data: { email: string }, context) => {
...
export const addEntry = regionalFunctions.https.onCall(
  async (
    data: {
      minutes: number;
      date: string;
