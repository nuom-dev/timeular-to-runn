...
export async function getApiKeys(userId: string): Promise<{apiKey: string; apiSecret: string}> {
    ...
    return {apiKey: userData.timeular?.apiKey ?? '', apiSecret: userData.timeular?.apiSecret ?? ''};
}
...
export async function entriesBetweenDates(token: string, startDate: Date, endDate: Date) {
  logger.log({startDate, endDate});
  const start = DateTime.fromMillis(startDate.getTime()).startOf('day').toISO()
    .split("+")[0];
  const end = DateTime.fromMillis(endDate.getTime()).endOf("day").toISO()
    .split("+")[0];
