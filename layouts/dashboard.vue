<template>
  <div :style="cssVars" class="h-screen bg-primary overflow-hidden flex font-primary">
    <notification-menu/>
    <Sidebar :logo="logo"/>
    <mobile-sidebar
      :logo="logo"
    ...
    <div class="relative flex-grow flex flex-col h-screen">
      <header-bar :show-on-desktop='false' @toggle="showSidebar = !showSidebar"/>
      ...
      >
        <span class="flex items-center mb-1">
          <mdicon name="copyright" size="12"/>{{ currentYear }} {{ platformName }}
...
@Component({
  components: {Sidebar, MobileSidebar, HeaderBar, LoadingOverlay, NotificationMenu}
})
export default class DashboardLayout extends Vue {
  ...
  @Action('config/bindConfigDocuments') bindConfigDocuments!: () => Promise<void>
