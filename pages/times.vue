<template>
  <div class="p-8">
    <h1 class="mb-4">
      Select the timeline to import entries from Timeular to Runn
    </h1>
    <div class="flex flex-col justify-around">
      <div class="my-2">
        <label class="mr-4">Start date</label>
        <input
          class="rounded-full text-secondary"
          type="date"
          :value="start"
          @input="setStartDate"
        />
...
export default class times extends Vue {
  ...
  @Action('timeular/getEntries') getEntries!: (date: {
    startDate: Date;
    endDate: Date;
  }) => Promise<void>;
  ...
  get timeEntries() {
    return this.entries.map((entry) => ({
      ...
      runnProject:
        this.projectsMap[this.projectsMapping[entry.activityId]] ?? null,
    }));
  }
  ...
  async addAll() {
    const filteredEntries = this.timeEntries.filter(
      (entry) => entry.runnProject?.id && !entry.saved
    );
